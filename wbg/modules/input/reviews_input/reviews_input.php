...
//		$OUT_filter = $free_filter->generate_filter_form();
		...
		$my_list->insert_cell("name","User", null, "width='10%'");
		$my_list->insert_cell("text","Review", null, "width='35%'");
		$my_list->insert_cell("email","E-mail", null, "width='15%'");
		$my_list->insert_cell("datums","Date", null, "width='15%'");
		$my_list->insert_cell("ip","IP", null, "width='10%'");
...
// [[[ Uzerskije funkcii
	...
	function _insert_doc_name($x , $data){

		$content = @mysql_result(mysql_query("SELECT content FROM ".$data['sql_table_name']." WHERE id=".$x), 0, 0);
		if (!$content) {
			$docName = 'Article has been deleted!';
		} else {
			$content = unserialize($content);
			$docName = $content['title'];
		}
		return $docName;
	}
